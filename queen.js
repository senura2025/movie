...
async function sessdl() {
  try {
    ...
    const base64Data = sess.SESSION_ID.split("Senura-MD")[1];
    if (!base64Data) {
...
async function connectToWA() {
  ...
  conn.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      let up = `
          🚀 **Senura MD 💚 Connected Successfully!** ✅ 
          
          --- **🎉 Welcome to Senura MD 💚!** 🎉 
      ...
      conn.sendMessage(conn.user.id, {
        text: up,
        contextInfo: {
          mentionedJid: ["94787751901@s.whatsapp.net"], // specify mentioned JID(s) if any
          groupMentions: [],
          forwardingScore: 999,
          isForwarded: true,
